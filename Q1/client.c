...
int main(void)
{
        ...
        gets(client_message);                                     //to get input from user of the client
        							//One is that gets() will only get character string data. 
                                                     //		will get only one variable at a time.
	 if(strcmp(client_message,sign_out_msg) == 0){ //comparing the char arrays, if return 0 then they are same, so checking if the 
	 //client requested to disconnect from the server then end the loop
         	loop = 0; 
